...
public class UserData extends JFrame {
    ...
    private String[] column = { "User Name", "Password", "Mobile no", "National ID", "Gmail", "Date and Time","Gender"};
    ...
    UserData() {
        ...
        try {
            ...
            for (int i = 0; i < totalLines; i++) {
                ...
                if (x.equals("User")) {
                    rows[0] = Files.readAllLines(Paths.get(file)).get(i).substring(10); // User Name
                    ...
                    rows[3] = Files.readAllLines(Paths.get(file)).get((i + 3)).substring(13); // Security Question
                    rows[4] = Files.readAllLines(Paths.get(file)).get((i + 4)).substring(7); // Answer
                    rows[5] = Files.readAllLines(Paths.get(file)).get((i + 5)).substring(12); // Date and Time
                    rows[6] = Files.readAllLines(Paths.get(file)).get((i + 5)).substring(8);
                    model.addRow(rows);
                }
            }
